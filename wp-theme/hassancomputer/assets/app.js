    ...
    .map(
      ...
      <article class="product" aria-label="${escapeHtml(p.title)}">
        <div class="product-media">
          ${p.img ? `<img class="product-img" src="${escapeHtml(p.img)}" alt="${escapeHtml(p.title)}" loading="lazy" />` : ""}
        ...
        <div class="product-foot">
          ...
          <button class="btn btn-primary btn-sm" type="button" data-open-product data-id="${escapeHtml(p.id)}">Add to Cart</button>
        </div>
      </article>
    `
    )
...
function bindSearch() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    window.location.href = `${pageUrl("home")}?s=${encodeURIComponent(q)}`;
  });
}
...
function bindSigninModal() {
  ...
  const setTab = (tab) => {
    ...
    qsa("[data-auth-pane]", modal).forEach((pane) => {
      pane.style.display = pane.getAttribute("data-auth-pane") === tab ? "block" : "none";
    });
  };
  qsa("[data-auth-tab]", modal).forEach((btn) => {
...
function bindAddToCart() {
  document.addEventListener("click", (e) => {
    ...
    if (openProduct) {
      ...
      if (id) window.location.href = `${pageUrl("product")}?id=${encodeURIComponent(id)}`;
      return;
    }
...
function setCheckoutItem(product, qty = 1) {
  if (!product) return;
  localStorage.setItem("hc_checkout_item", JSON.stringify({ id: product.id, qty: Number(qty) || 1 }));
}
...
function getCheckoutTotals() {
  ...
  return { item, subtotal, shipping, platformFee, total: subtotal + shipping + platformFee };
}
...
function renderProductPage() {
  ...
  if (!product) {
    wrap.innerHTML = `<div class="card"><h1 class="h3">Product not found</h1><a class="btn btn-primary" href="${escapeHtml(pageUrl("products"))}">Back to products</a></div>`;
    return;
  }
  wrap.innerHTML = `
    <article class="pd-grid">
      <div class="card pd-image-wrap">
        <img class="pd-image" src="${escapeHtml(product.img || "")}" alt="${escapeHtml(product.title)}" />
      </div>
...
function bindProductPage() {
  document.addEventListener("click", (e) => {
    ...
    if (buy) {
      ...
      window.location.href = pageUrl("checkoutDelivery");
    }
  });
}
...
function bindDeliveryForm() {
  ...
  if (!totals) {
    window.location.href = pageUrl("products");
    return;
  }
  ...
  form.addEventListener("submit", (e) => {
    ...
    setDeliveryInfo(Object.fromEntries(fd.entries()));
    window.location.href = pageUrl("checkoutReview");
  });
}
  ...
  wrap.innerHTML = `
    <div class="card co-left">
      ...
      <div class="co-package">
        <img class="cart-thumb" src="${escapeHtml(totals.item.product.img || "")}" alt="${escapeHtml(totals.item.product.title)}" />
        <div>
...
function bindCheckoutReview() {
  document.addEventListener("click", (e) => {
    ...
    if (!proceed) return;
    window.location.href = pageUrl("checkoutPayment");
  });
}
...
function getActivePaymentMethod() {
  const active = qs("[data-payment-tabs] [data-method].is-active");
  return active?.getAttribute("data-method") || "cod";
}

async function placeOrder(paymentMethod) {
  const totals = getCheckoutTotals();
  if (!totals || !DATA.ajaxUrl) return false;
  const delivery = getDeliveryInfo();
  const order = {
    delivery,
    payment: paymentMethod,
    total: totals.total,
    items: [{ id: totals.item.product.id, title: totals.item.product.title, qty: totals.item.qty, price: totals.item.product.price }],
  };
  const body = new FormData();
  body.append("action", "hc_place_order");
  ...
  try {
    ...
    if (json.success) {
      localStorage.removeItem("hc_checkout_item");
      localStorage.removeItem("hc_delivery_info");
      localStorage.removeItem("hc_cart");
      return true;
    }
    toast(json.data?.message || "Could not place order.");
  } catch {
...
function bindPaymentTabs() {
  ...
  tabsWrap.addEventListener("click", (e) => {
    ...
    if (!btn) return;
    setMethod(btn.getAttribute("data-method") || "cod");
  });
  panes.addEventListener("submit", async (e) => {
    e.preventDefault();
    const method = getActivePaymentMethod();
    if (method === "installment") {
      toast("Installment is not available yet.");
      return;
    }
    const ok = await placeOrder(method);
    if (ok) window.location.href = pageUrl("orderSuccess");
  });
  setMethod("easypaisa");
}
...
function renderCartPage() {
  ...
  if (entries.length === 0) {
    root.innerHTML = `
      <div class="empty-cart">
        ...
        <a class="btn btn-primary" href="${escapeHtml(pageUrl("products"))}">Continue shopping</a>
      </div>
    `;
    ...
    .map(([id, qty]) => {
      ...
      return `
        <div class="cart-row" data-cart-row="${escapeHtml(id)}">
          <div class="cart-prod">
            <img class="cart-thumb" src="${escapeHtml(p.img || "")}" alt="${escapeHtml(p.title)}" />
            <div>
          ...
          <button class="cart-remove" type="button" aria-label="Remove" data-remove="${escapeHtml(id)}">Remove</button>
        </div>
      `;
    })
    .join("");
  ...
  if (shipEl) shipEl.textContent = formatPKR(0);
  if (totEl) totEl.textContent = formatPKR(subtotal);
}
...
function bindCartPage() {
  document.addEventListener("click", (e) => {
    ...
    if (checkout) {
      const cart = getCart();
      const firstId = Object.keys(cart).find((id) => Number(cart[id]) > 0);
      const product = firstId ? getProductById(firstId) : null;
      if (product) setCheckoutItem(product, cart[firstId] || 1);
      window.location.href = pageUrl("checkoutDelivery");
    }
  });
}
...
async function subscribeEmail(email) {
  if (!DATA.ajaxUrl) return false;
  const body = new FormData();
  body.append("action", "hc_subscribe");
  body.append("nonce", DATA.nonce || "");
  ...
  try {
    ...
    const json = await res.json();
    if (json.success) {
      toast(json.data?.message || "Subscribed successfully.");
      return true;
    }
    toast(json.data?.message || "Could not subscribe.");
  } catch {
    toast("Network error. Please try again.");
  }
  return false;
...
function bindForms() {
  qsa("[data-subscribe]").forEach((sub) => {
    sub.addEventListener("submit", async (e) => {
      ...
      const ok = await subscribeEmail(v);
      if (ok) sub.reset();
    });
  });
}
